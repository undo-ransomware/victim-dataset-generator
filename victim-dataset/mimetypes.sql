...
-- update files set mimetype=(select ext from mmap where "to"=mimetype);
...
select mmap.ext as ext, cast(dbb as integer) as dbb, sum(count) as count
	from files join mmap on mimetype = "to" where "from" like '%$%' and dbb >= 0
